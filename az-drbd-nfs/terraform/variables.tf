...
variable "cluster_hostnames" {
  description = "hostnames for cluster node vms"
  type        = list(string)
  default     = ["node0", "node1"]
}

variable "cluster_vm_size" {
  description = "vm_size for cluster node vm"
  type        = string
  default     = "Standard_B1s"
  #default     = "Standard_B2ms"
}

variable "disk_size_gb_data0" {
  description = "size in GB for cluster node vm data disk 0"
  type        = number
  default     = 32
}

variable "cluster_image" {
  description = "storage image reference for cluster node vms"
  type        = map(string)
